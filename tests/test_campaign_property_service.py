"""Tests for the /property-detail webhook's resolution logic - the
actual bug found on 31 Jul 2026: campaign_context/the WATI flow only
ever have a projectCode/projectName-style value, never the backend's
internal `id`, but resolve_campaign_property() used to call ONLY
fetch_project(code) (which needs `id`) with no fallback - so it would
silently return "not found" for every real lead."""
import pytest
